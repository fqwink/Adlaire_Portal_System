...
### 1.4 バージョニング方針
本システムのバージョン番号は **`v0.N`（Nは1から始まる単純な累積連番）** を採用する。

- **Nは変更を加えるたびに1ずつ増加する。** 破壊的変更（DBスキーマ変更・実行環境変更等）か、機能追加か、
  誤字修正等の軽微な変更かによって桁や増分を使い分けない（SemVer（`MAJOR.MINOR.PATCH`）のような重大度による
  区別は採用しない）。1エントリ＝1バージョンの原則で、§10の変更履歴の行数と一致する
- **Nはいかなる理由でもリセットしない。** 常に積み上げる一方向の連番とし、減らす・巻き戻す・1から数え直す
  操作は理由の如何を問わず禁止とする。将来、先頭の`0`を`1`に切り替える場合（下記）が生じても、Nはその時点の
  値から連続して増加し続ける（例:`v0.99`の次を`v1.0`ではなく`v1.100`とする、のようにNを継続する）
- **先頭の`0`は固定**とする。将来、システムが「安定版」と呼べる一区切りに達したと判断した場合に`1`への切り替えを
  検討する余地は残すが、現時点では固定とし、切り替えの基準自体は本ファイル未策定。切り替えた場合もNのリセットは
  行わない（上記参照）
