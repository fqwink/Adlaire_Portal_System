...
理由の如何を問わず一切禁止とする。** お気に入り、クリック履歴、閲覧設定（ダークモード手動切り替え等）の記憶、
下書きの自動保存など、実現に「ブラウザ内保存」を前提とする機能は提案・実装しない。

理由: 本システムは §1「システム概要」の通り、サーバー・データベースを持たず、`src/portal-config.json`という
単一のソースコードのみを状態の実体とする設計を意図的に選んでいる。ブラウザ内保存を許すと、閲覧者ごと・端末ごとに
異なる「隠れた状態」が生まれ、「ソースコードを見れば全ての表示内容が再現できる」という静的サイトとしての
再現性・シンプルさが失われる。新機能を検討する際は、この制約を先に確認し、ブラウザ内保存なしで実現できる形に
限定すること。
