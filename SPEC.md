...
**編集者があらかじめ`portal-config.json`に指定する、全閲覧者共通の静的データ**として代替できる場合がある。
機能要件を「個人ごとの状態」から「編集者が管理する共有データ」に置き換えられないか、まず検討すること。
...
interface LinkItem {
  ...
  url: string;
  icon: string;
  pinned?: boolean; // 省略時はfalse扱い。§5.3参照
}
...
| ピン留め表示 | 全カテゴリを横断して`pinned: true`のリンクだけを集め、ページ最上部（お知らせ欄より上）に専用のカードグリッドとして表示する。0件の場合は欄ごと非表示。§1.3の制約により、これは**編集者が`portal-config.json`で指定した全閲覧者共通の表示**であり、閲覧者ごとに変化する個人のお気に入りではない |
...
JSONファイルをGUI操作で組み立てる、ローカル専用の補助ツール」という位置づけである。
...
| ピン留め切り替え | 各リンクの「📌」ボタンで`pinned`を`true`/`false`にトグルする。ピン留め中はボタンがテーマカラーで強調表示され、プレビューの最上部にピン留め欄として反映される |
...
| バージョン | 日付 | 内容 |
|---|---|---|
| 4.3 | 2026-09-03 | 「ピン留めリンク」機能を追加。`LinkItem`に任意項目`pinned`(真偽値)を新設し、`true`のリンクを閲覧画面最上部に共通表示する。§1.3のLocalStorage禁止制約と両立する代替案として、個人ごとの状態ではなく編集者が`portal-config.json`で管理する共有データとして実装。編集補助ツールにピン留め切り替えボタンを追加 |
