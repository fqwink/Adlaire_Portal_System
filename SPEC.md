...
| バージョン | 日付 | 内容 |
|---|---|---|
| 5.4 | 2026-09-03 | 認証機能を除く運用・機能改善8点を追加。①アクセスログ/エラーログを`data/access.log`と標準出力へ記録(§8)、②`data/portal.db`の定期バックアップ(既定60分間隔、直近24世代保持。§2.3)、③死活監視用`GET /healthz`(§4.5)、④リンクの匿名クリック数集計(`link_clicks`テーブル・`POST /api/click`・両画面での表示。§3.5, §4.6)、⑤保存のたびのスナップショットを記録する変更履歴(`change_log`テーブル・`GET /api/history`・`GET /api/history/:id`・編集画面の履歴モーダルからの内容確認/読み込み。直近50件保持。§3.6, §4.7, §5.2.2)、⑥閲覧画面の検索結果ハイライト表示(`<mark>`タグ。§5.1.2)、⑦閲覧画面のカテゴリ折りたたみ表示(DOM上の一時的な表示のみで永続化はしない。§1.3, §5.1.2)、⑧編集画面でのカテゴリ/リンクのドラッグ&ドロップ並び替え(既存の⬆️⬇️ボタンと併存。リンクは同一カテゴリ内のみ。§5.2.2)を実装。`LinkItem`に読み取り専用の任意項目`clicks`を追加(§3.7, §3.8) |
