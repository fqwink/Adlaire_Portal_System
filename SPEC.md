...
実現例: 「お気に入り」に相当する要件は、閲覧者ごとの個人状態としては実現不可能だが、§5.1.2「ピン留め表示」のように
**編集者があらかじめデータベース（`portal-config.json`のシード値、および編集画面での編集内容）に指定する、
...
| pinned | INTEGER | NOT NULL DEFAULT 0 | ピン留めフラグ（0/1。§5.1.2参照） |
  ...
  "categories": [
    {
      "name": "カテゴリ名",
      "links": [
        { "name": "リンク名", "url": "https://example.com", "icon": "🔍", "pinned": true }
      ]
    }
  ]
}
...
`pinned` は省略可能で、省略時は `false` として扱う。
...
- **処理**: サーバー側で §6 のバリデーションを実施した後、`settings`を更新し、`news`/`categories`/`links`を全削除してから再挿入する（トランザクション内で実行、失敗時はロールバック）
- **成功時**: `200 OK`、更新後の設定をJSONで返す
- **失敗時**: バリデーションエラー時は `400 Bad Request`、`{ "error": "<エラーメッセージ>" }`

### 4.3 `POST /api/config/reset`
データベースの内容を `src/portal-config.json` の内容にリセットする（`PUT`と同じ置き換え処理をシードデータに対して実行）。
...
- **メインエリア**:
  - エラー表示欄（API取得失敗時のみ表示）
  - ピン留め欄（`pinned: true`のリンクが1件以上ある場合のみ表示）
...
| ピン留め表示 | 全カテゴリを横断して`pinned: true`のリンクだけを集め、ページ最上部（お知らせ欄より上）に専用のカードグリッドとして表示する。0件の場合は欄ごと非表示。§1.3の制約により、これは**編集者がデータベースで指定した全閲覧者共通の表示**であり、閲覧者ごとに変化する個人のお気に入りではない |
...
| ピン留め切り替え | 各リンクの「📌」ボタンで`pinned`を`true`/`false`にトグルする。ピン留め中はボタンがテーマカラーで強調表示され、プレビューの最上部にピン留め欄として反映される（保存するまでDBには反映されない） |
...
| `categories[].links[].pinned` | 指定する場合は真偽値(`true`/`false`)であること（省略可、省略時は`false`扱い） |
...
| リンク | カテゴリに属する個別の外部/内部URL。名称・URL・アイコン・ピン留めフラグを持つ |
| ピン留め | 編集者が指定する、全閲覧者共通の「よく使うリンク」表示。個人ごとのお気に入りとは異なる（§1.3参照） |
